...
Usage:
python training_topic.py

OR
python training_topic.py pretrained_transformer_model_name
"""
...
model_save_path = f'output/training_wiki_topics_{num_epochs}_'+model_name.replace("/", "-")+'-'+datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
...
train_samples = []
possible_labels = set()
prefix = "datasets/topic"
for split in ("train", "dev", "test"):
    # TODO: find some way to use dev and test during initial word-level training
    if split != "train":
        continue
    data_path = f"{prefix}_{split}_wiki.json"
    with open(data_path, "r") as data:
        for json_str in data:
            json_obj = json.loads(json_str)
            sentences = sent_tokenize(json_obj['text'])
            if len(sentences) >= 2:
            ...
            train_samples.append(InputExample(texts=[text_a, text_b], label=label))
'''
